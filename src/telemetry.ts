 ...
 *
 * Authority: klappy://canon/specs/appbuilder-mcp-v1-spec §6
...
export const SELF_REPORT_HEADERS: Record<string, string> = {
  ...
  "x-appbuilder-client-version": "Version string (semver recommended)",
  "x-appbuilder-agent-name": "AI agent or model name",
  "x-appbuilder-agent-version": "Version string for the agent/model",
  "x-appbuilder-surface":
    "Where this is running ('claude.ai', 'vscode', 'cli', 'production')",
  "x-appbuilder-contact-url": "Project or organization URL",
  "x-appbuilder-policy-url": "Your privacy or telemetry policy URL",
  "x-appbuilder-capabilities":
 ...
 */
export const MINIMAL_POLICY = `# AppBuilder MCP Telemetry — Minimal Policy

Dataset: appbuilder_telemetry (Cloudflare Workers Analytics Engine)
...
For the full governance document, see:
  ...
  https://github.com/klappy/appbuilder-mcp/blob/main/canon/governance/telemetry-governance.md`;
...
export interface TelemetryEnv {
  APPBUILDER_TELEMETRY?: AnalyticsEngineDatasetLike;
 ...
 *
 * Positions for blobs (12) and doubles (10) come from `telemetry-schema.ts`
 * which is the single source of truth. `buildBlobsArray` / `buildDoublesArray`
 * assemble in BLOB_SCHEMA / DOUBLE_SCHEMA order from a name-keyed map, so
 * callers cannot accidentally transpose positions.
 */
export function writeTelemetry(
  ...
  if (!env.APPBUILDER_TELEMETRY) return;
  ...
  env.APPBUILDER_TELEMETRY.writeDataPoint({
    blobs,
    doubles,
 ...
 *   2. x-appbuilder-client header    → source: "header"
...
export function resolveConsumer(
  ...
  const headerClient = headers.get("x-appbuilder-client");
  if (headerClient) {
...
/**
 * Validate that a SQL query targets ONLY the appbuilder_telemetry dataset.
 *
 * 1. Strip comments and string literals (prevent smuggling via literals).
 * 2. Extract all FROM/JOIN targets.
 * 3. Require at least one target and all targets must be appbuilder_telemetry.
 */
export function validateDatasetAllowlist(sql: string): boolean {
  ...
  return (
    ...
    datasetRefs.every((name) => name === "appbuilder_telemetry")
  );
}
 ...
 *   1. Dataset allowlist — only appbuilder_telemetry
...
export async function forwardTelemetryQuery(
  ...
  // Pre-step: rewrite semantic field names → positional column refs.
  // Idempotent — callers using `blob1`/`double2` directly are unaffected.
  // Authority: klappy://canon/specs/appbuilder-mcp-v1-spec §3 telemetry_public.
  const rewritten = rewriteSemanticSql(sql);

  // Guard 1: dataset allowlist
  if (!validateDatasetAllowlist(rewritten)) {
    return sanitizedError(
      "Query must reference only dataset appbuilder_telemetry",
    );
  }
