/**
 * telemetry-schema.ts — SINGLE source of truth for appbuilder_telemetry blob/double positions.
 *
 ...
 *
 * Provenance: structurally derived from ptxprint-mcp/src/telemetry-schema.ts
 * @ 4271d700ffd290034190cc0751726fdd29e5fe0c. Module API matches verbatim.
 * Dataset name and SAB-specific descriptions are the only deltas: dataset is
 * `appbuilder_telemetry`; tool_name examples reference `submit_build`; bytes_out
 * is APK byte count; passes_completed / overfull_count / pages_count slots are
 * unused in SAB and kept at position for schema stability (position is forever).
 */
...
export const BLOB_SCHEMA = [
  ...
  { name: "cache_outcome", desc: "hit | miss | n/a (mcp_request when tool_name = submit_build)" },
...
export const DOUBLE_SCHEMA = [
  ...
  { name: "bytes_out", desc: "response body size (mcp_*) or APK byte count (job_terminal success)" },
  { name: "sources_count", desc: "number of source URLs in payload (submit_build)" },
  { name: "fonts_count", desc: "number of font URLs in payload (submit_build) — typically 0 in SAB" },
  { name: "figures_count", desc: "number of figure URLs in payload (submit_build) — typically 0 in SAB; icons live elsewhere" },
  { name: "passes_completed", desc: "build pass count when the job stopped (legacy from PTXprint autofill; reserved for SAB; position is forever)" },
  { name: "overfull_count", desc: "Overfull \\hbox warnings (PTXprint-specific; unused in SAB; position is forever)" },
  { name: "pages_count", desc: "page count of produced PDF (PTXprint-specific; unused in SAB — APKs have no page count; position is forever)" },
] as const;
