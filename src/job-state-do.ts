 ...
 *
 * Per canon/specs/appbuilder-mcp-v1-spec.md §5. One DO instance per
 * `job_id` (idFromName(job_id)). State persists in SQLite-backed DO storage
 * so it survives Worker invocations and Container restarts.
 *
 ...
 *
 * Provenance: structurally identical to
 * klappy://canon/specs/ptxprint-mcp-v1.2-spec §5 / ptxprint-mcp/src/job-state-do.ts.
 * The only AppBuilder-specific delta is the artifact key naming
 * (apk_r2_key instead of pdf_r2_key). Everything else is copied.
 */
  ...
  async fetch(req: Request): Promise<Response> {
    ...
    if (req.method === "POST" && url.pathname === "/cancel") {
      ...
      const merged: JobState = {
        ...
        human_summary:
          "Cancellation requested. v0.1 PoC: flag recorded but the SAB CLI does not currently support mid-build cancellation; the build completes before stopping.",
      };
      await this.state.storage.put("job", merged);
