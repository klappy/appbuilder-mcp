 ...
 *     archived. They mirror the article's "What Gets Snapshotted" enumeration as
 *     adapted for Scripture App Builder (APK bytes replace PDF pages; submit_build
 *     replaces submit_typeset).
 ...
 *   - The module holds NO SAB domain opinions beyond what the canon article
 *     already specifies.
 *
 * Provenance: structurally derived from
 * klappy/ptxprint-mcp/src/snapshot.ts @ 4271d70. Module API matches verbatim.
 * SAB-specific deltas: dataset is `appbuilder_telemetry`; blob3 filter uses
 * `submit_build`; the lifetime hero stat is APK bytes (double4 = bytes_out)
 * rather than PDF pages (PTXprint's double10 = pages_count, which in
 * appbuilder is reserved at position with no APK analog).
 *
 * Authority:
...
export interface SnapshotMetric {
  name: string;
  objectKey: string;
  /** SQL template — must reference only `appbuilder_telemetry`. */
...
//
//  Per parity-spec §4 P2.8 — at least four SAB-equivalent metric
//  definitions covering: build count, success rate, failure-mode
//  distribution, lifetime cumulative.
//
...
//    3. `failure_mode_distribution_weekly` — per-failure-mode breakdown
//    4. `apk_bytes_weekly`        — lifetime cumulative (sum of bytes_out
//                                    on success; the SAB hero stat)
//
//  Schema reference (mirrors src/telemetry-schema.ts BLOB_SCHEMA / DOUBLE_SCHEMA):
//    blob1 = event_type, blob3 = tool_name, blob8 = failure_mode,
//    blob9 = cache_outcome, double4 = bytes_out.
//  Hardcoded here intentionally — ptxprint-mcp does the same in its snapshot
//  module. The schema-pinning test in test/telemetry-schema.test.ts catches
//  any drift on the schema side; this module's SQL must be updated in
//  lockstep if positions ever shift (they won't — position is forever).
// ────────────────────────────────────────────────────────────

const SOURCE_DATASET = "appbuilder_telemetry";

export const METRICS: SnapshotMetric[] = [
  {
    name: "total_builds_weekly",
    objectKey: "total-builds-weekly.jsonl",
    buildSql: (start, end) => `
      SELECT SUM(_sample_interval) AS value
      FROM appbuilder_telemetry
  ...
  {
    name: "apk_bytes_weekly",
    objectKey: "apk-bytes-weekly.jsonl",
    buildSql: (start, end) => `
      SELECT SUM(double4 * _sample_interval) AS value
      FROM appbuilder_telemetry
      WHERE blob1 = 'job_terminal'
        AND blob8 = 'success'
        AND timestamp >= toDateTime('${start} 00:00:00')
...
export interface LifetimeHeroStat {
  lifetime_apk_bytes: number;
  archive_apk_bytes: number;
  current_week_apk_bytes: number;
...
/**
 * Compose lifetime APK bytes from:
 *   - sum of value across all rows in apk-bytes-weekly.jsonl (archive)
...
export async function getLifetimeHeroStat(
  ...
  // Archive: sum value across all rows in apk-bytes-weekly.jsonl
  const archive = await readSnapshotJsonl(env, "apk-bytes-weekly.jsonl");
  const archiveRows = archive.filter(
    (r) => r.metric === "apk_bytes_weekly",
  );
  const archiveBytes = archiveRows.reduce((sum, r) => sum + (r.value ?? 0), 0);
  ...
  const currentWeekBytes = numericValue(rows[0], "value");

  return {
    lifetime_apk_bytes: archiveBytes + currentWeekBytes,
    archive_apk_bytes: archiveBytes,
    current_week_apk_bytes: currentWeekBytes,
