 ...
 * Hosting is the agent's concern, upstream of MCP.
 *
 * Internal routes:
 *   POST /internal/job-update   — container state patches
 *   POST /internal/telemetry    — container telemetry forwarding
 *   POST /internal/upload       — container R2 artifact upload
 *   GET  /internal/job-cancel-flag — container polls for cancellation
 *   GET  /r2/<key>              — public Worker proxy for R2 reads
 *
 * Telemetry hooks:
 *   - mcp_request event emitted at every MCP request
 *   - tool_call event emitted when method == "tools/call"
 *
 * Streamable HTTP MCP transport is mounted at /mcp; SSE at /sse.
 *
 * Provenance: structurally derived from ptxprint-mcp/src/index.ts. The 6-tool
 * surface, internal routes, and telemetry hooks are identical in shape;
 * AppBuilder-specific changes are tool/payload naming (submit_build vs.
 * submit_typeset, apk vs. pdf), tool descriptions, and the telemetry blob
 * rebinding documented in canon/governance/telemetry-governance.md.
 */
...
import { outputApkKey, outputLogKey } from "./output-naming.js";
import {
  initJob,
  readJobState,
...
export { AppbuilderContainer } from "./container.js";
...
interface Env {
  ...
  APPBUILDER_CONTAINER: DurableObjectNamespace;
  OUTPUTS: R2Bucket;
  WORKER_URL: string;
  BUILD_TIMEOUT_DEFAULT: string;
  RESULT_PRESIGNED_TTL: string;
  // v1.3 telemetry bindings
  APPBUILDER_TELEMETRY: AnalyticsEngineDataset;
...
export class AppbuilderMcp extends McpAgent<Env> {
  ...
  async init() {
    ...
    this.server.tool(
      "submit_build",
      "Submit a Scripture App Builder build job. Validates the payload, computes its sha256, and either returns the cached APK URL (if the same payload has been seen before) or dispatches the job to a Container worker. Returns immediately with a job_id; poll get_job_status for state and the result APK URL.",
      { payload: PayloadSchema },
      async ({ payload }: { payload: Payload }) => {
        const submittedAt = new Date().toISOString();
        const hash = await computePayloadHash(payload);
        const apkKey = outputApkKey(payload, hash);
        ...
        // to /internal/upload), and to build the predicted_apk_url returned
        // to the agent.
        //
        // Priority order:
        ...
        const head = await env.OUTPUTS.head(apkKey);
        if (head) {
          return {
            content: [
              {
                ...
                text: JSON.stringify(
                  {
                    ...
                    predicted_apk_url: r2PublicUrl(apkKey, baseUrl),
                    cached: true,
                    payload_hash: hash,
                  },
                  null,
        ...
        const containerId = env.APPBUILDER_CONTAINER.idFromName(jobId);
        const containerStub = env.APPBUILDER_CONTAINER.get(containerId);
    ...
    this.server.tool(
      ...
      async ({ job_id }: { job_id: string }) => {
        ...
        const augmented = {
          ...state,
          apk_url: state.apk_r2_key ? r2PublicUrl(state.apk_r2_key, baseUrl) : null,
    ...
    this.server.tool(
      "cancel_job",
      "Request cancellation of a running job. Sets a flag in the JobStateDO that the running Container polls every 10 seconds. Container-side SIGTERM dispatch is a Day-2 enhancement; for Day-1 the flag is recorded but AppBuilder completes its current pass before stopping.",
      { job_id: z.string().min(1) },
      async ({ job_id }: { job_id: string }) => {
        const env = this.env;
    ...
    // Authority: klappy://canon/specs/appbuilder-mcp-v1-spec §3 telemetry_policy
    this.server.tool(
      "telemetry_policy",
      "Returns the AppBuilder MCP telemetry governance policy, the self-report header dictionary, and the three-tier fallback chain status. No parameters required.",
      {},
      async () => {
                  ...
                  fallback_chain: [
                    {
                      tier: "knowledge_base",
                      source:
                        "https://raw.githubusercontent.com/klappy/appbuilder-mcp/main/canon/governance/telemetry-governance.md",
                    },
                    {
                      tier: "bundled",
    ...
    //
    // Public SQL query forwarder over the appbuilder_telemetry Analytics Engine
    ...
    this.server.tool(
      ...
      "Query the appbuilder_telemetry Analytics Engine dataset with SQL. The data is public — this is the same dashboard the maintainer sees. You may use semantic field names directly (e.g. `WHERE event_type = 'tool_call'`); the worker rewrites them to positional refs (`blob1` / `double2`) before forwarding to Analytics Engine. Call telemetry_schema() to discover the full position-to-name mapping. Use SUM(_sample_interval) instead of COUNT(*) for sample-correct totals. Rate-limited to 60 queries/consumer/hour. See telemetry_policy() for canned query examples.",
      {
        sql: z
          .string()
          .min(1)
          .describe("A read-only SQL query against appbuilder_telemetry."),
      },
...
export default {
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // Internal: container PUTs artifacts (APK, log) into R2.OUTPUTS.
...
const mcpHandler = AppbuilderMcp.serve("/mcp", { binding: "MCP_AGENT" });
const mcpSseHandler = AppbuilderMcp.serveSSE("/sse", { binding: "MCP_AGENT" });
...
async function emitTelemetryForMcp(
      ...
      if (contentType.includes("json") || contentType === "") {
        ...
        if (resultText) {
          ...
          if (rpc.toolName === "submit_build") {
            ...
            if (payload) {
              // For appbuilder-mcp v0.1, the existing AE blob slots are repurposed:
              //   sources_count → icons count
              //   fonts_count   → 1 if caller-supplied keystore, 0 if using bundled debug
              //   figures_count → 1 if -build-modern-pwa requested, 0 otherwise
              // Slot names retained verbatim for ptxprint-mcp dashboard portability;
              // canon/governance/telemetry-governance.md when authored will document the rebinding.
              sourcesCount = Array.isArray(payload.icons) ? payload.icons.length : 0;
              fontsCount = payload.keystore ? 1 : 0;
              figuresCount = payload.build_modern_pwa ? 1 : 0;
            }
          }
  ...
  writeTelemetry(telEnv, "mcp_request", {
    ...
    cache_outcome:
      rpc.toolName === "submit_build" ? cacheOutcome || "n/a" : "",
